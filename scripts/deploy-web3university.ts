...
async function main() {
  ...
  if (!deployer || !deployer.account) {
    throw new Error(
      'No wallet client with an account is available. Configure accounts for the selected network.'
    )
  }
  ...
  console.log('   - 账户:', deployerAddress)
  ...
  try {
    // 1. 部署 LueToken
    console.log('\n🪙 1/4 部署 LueToken...')
    const lueToken = await viem.deployContract('LueToken', [
      'LueLueLue', // name
      'Lue', // symbol
    ...
    const tokenExchange = await viem.deployContract('TokenExchange', [
    ...
    const coursePurchase = await viem.deployContract('CoursePurchase', [
      lueToken.address, // _lueToken
    ...
    const courseCertificate = await viem.deployContract('CourseCertificate', [
    ...
    const tokenInfo = (await lueToken.read.getTokenInfo()) as readonly [
      string,
      string,
      bigint,
      bigint,
    ...
    const exchangeBalance = (await lueToken.read.balanceOf([
    ...
    console.log(
      `npx hardhat verify --network ${networkName} ${lueToken.address} "LueLueLue" "Lue" 1000000 0 100000000 "${deployer.account.address}"`
    )
    console.log(`\n# TokenExchange`)
    console.log(
      `npx hardhat verify --network ${networkName} ${tokenExchange.address} "${
        lueToken.address
      }" "${parseUnits('0.0001', 18)}" 10000 6 6`
    )
    console.log(`\n# CoursePurchase`)
    console.log(
      `npx hardhat verify --network ${networkName} ${coursePurchase.address} "${lueToken.address}" 250`
    )
    console.log(`\n# CourseCertificate`)
    console.log(
      `npx hardhat verify --network ${networkName} ${courseCertificate.address} "Web3 University Certificate" "W3UC"`
    )
    console.log('```')
    ...
    console.log('\n🔗 Etherscan 链接:')
    console.log(
      `   - LueToken: https://sepolia.etherscan.io/address/${lueToken.address}`
    )
    console.log(
      `   - TokenExchange: https://sepolia.etherscan.io/address/${tokenExchange.address}`
    )
    console.log(
      `   - CoursePurchase: https://sepolia.etherscan.io/address/${coursePurchase.address}`
    )
    console.log(
      `   - CourseCertificate: https://sepolia.etherscan.io/address/${courseCertificate.address}`
    )

...
async function main() {
  ...
  try {
    ...
    console.log("   ✅ YD代币合约部署成功:", ydTokenAddress);
    ...
    for (const course of sampleCourses) {
      ...
      await createTx.wait();
      console.log(`   ✅ 创建课程 ${course.id}, 价格: ${course.price} YD`);
    }
    ...
    console.log(`npx hardhat verify --network ${network.name} ${exchangeContractAddress} "${ydTokenAddress}" "${mockUsdtAddress}" "${deployer.address}"`);
